---
title: "Direct3D : Direct3D 초기화"
excerpt: "Direct3D 사용을 위한 프레임워크 확장"
toc: true
toc_sticky: true
toc_label: "목차"
categories:
...
이전에 구성한 프래임워크에 Direct3D 기능을 가지는 클래스를 추가하여 확장할 것이다. GraphicsClass 하위에 D3DClass를 만들것이다.
...
warning C4316이란 비주얼 스튜디오에서 제공되는 new, delete 명령어는 8byte 정렬을 사용하는데, XMMATRIX 변수의 경우 16byte 정렬을 사용하도록 명시하고 있으므로, HEAP 할당시에 컴파일러는 보장하지 않을 것으라는 경고를 전달한다.

AlignedAllocationPolicy.h는 이 해결 방법으로 new, delete를 재정의하여 사용하는 것으로 해결했다. 이 경고를 해결하지 않으면 정렬시 데이터를 안전하게 보장받을 수 없을 가능성이 생긴다.
...
멤버 변수로 D3DClass를 만들어주어 해당 클래스 내에서 사용할 수 있도록한다. D3DClass는 DirectX에 필요한 기본적인 기능들을 모두 담당하게 된다.
...
D3DClass 멤버 변수가 있기 때문에 GraphicsClass에서 해당 객체를 초기화하고 정리하는 코드를 넣었고, Render 함수에 BeginScene과 EndScene을 호출하는 부분을 넣어 Direct3D를 사용하여 그리는 부분도 추가할 것이다.
...
D3DClass는 생성자, 소멸자 그리고 장면 그리기 함수 부분들과 디바이스에 대한 부분들을 다룬다. 
...
이제 해당 기능들을 통해 Direct3D를 초기화하고 종료 할 수 있게 되었다.